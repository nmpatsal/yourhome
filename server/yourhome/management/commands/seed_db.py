...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        yourhome_models = inspect.getmembers(models, inspect.isclass)
        to_seed = None
        for yourhome_model in yourhome_models:
            if yourhome_model[0] == options['model']:
                to_seed = yourhome_model[1]
                break
        if not to_seed:
            return
